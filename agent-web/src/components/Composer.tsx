...
export default function Composer({
  ...
  useKeybindAction(
    ...
    (actionId) => {
      if (actionId === 'chat.focusComposer') {
        ref.current?.focus()
        return true
      }
      if (actionId === 'chat.openModes') {
        setModesOpen(true)
        return true
      }
      if (actionId === 'chat.togglePlanMode') {
        toggleMode('plan')
        return true
      }
      if (actionId === 'chat.toggleResearchMode') {
        toggleMode('research')
        return true
      }
      if (actionId === 'chat.toggleDebugMode') {
        toggleMode('debug')
        return true
      }
      if (actionId === 'chat.compressConversation') {
        if (compressing || disabled || !conversationId) return false
        void handleCompress()
        return true
      }
      if (actionId === 'chat.addGithubContext') {
