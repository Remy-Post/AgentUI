...
import { useStreamingStore, type MemoryRecallEvent, type ToolEvent } from '../store/streaming'
...
function formatToolEvent(event: ToolEvent): string {
  const parts = [event.tool_name]
  if (event.agent_name) parts.push(event.agent_name)
  else if (event.task_id) parts.push(event.task_id)
  return parts.join(' · ')
}
...
export default function RunInspector({
  ...
  const combinedTools = useMemo(() => {
    ...
    const live = streaming.toolEvents.map(formatToolEvent)
  ...
  const activeSubagent = useMemo(() => {
    for (let i = streaming.toolEvents.length - 1; i >= 0; i -= 1) {
      const event = streaming.toolEvents[i]
      const label = event.agent_name ?? event.task_id ?? event.description
      if (!label) continue
      return {
        label: truncate(label, 64),
        status: event.status ?? (isStreamingHere ? 'running' : 'seen')
      }
    }
    return null
  }, [streaming.toolEvents, isStreamingHere])
      ...
      <div className="rail-section">
        ...
        {activeSubagent ? (
          <div className="rail-agent">
            <span className="mono">{activeSubagent.label}</span>
            <span className="chrome">{activeSubagent.status}</span>
          </div>
        ) : (
